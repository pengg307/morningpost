...
- **职责**: 负责数据库设计、数据获取函数实现、代理Fallback逻辑
- **技能**: Python, SQLite, API集成, 错误处理
- **输出**: 数据库初始化脚本, 数据获取函数, 代理管理模块

### 2. 数据分析师 (Data Analyst)
- **职责**: 负责数据规整逻辑、质量评分算法、数据一致性检查
- **技能**: 数据分析, 统计学, 数据质量评估
- **输出**: 数据规整算法, 质量评分模型, 数据清洗脚本

### 3. 系统集成专家 (Integration Specialist)
- **职责**: 负责定时任务配置、晨报数据准备、通知机制
- **技能**: 定时任务, 系统集成, 通知机制
- **输出**: 定时任务脚本, 晨报数据接口, 通知模块

### 4. 晨报撰稿人 (Report Writer)
- **职责**: 负责从规整数据生成晨报可用格式
- **技能**: 数据处理, 格式化输出
- **输出**: 晨报数据准备脚本, 数据格式化模块
...
1. **使用CrewAI框架** - 按照CrewAI的最佳实践实现
2. **角色分工明确** - 每个Agent职责清晰
3. **任务依赖合理** - 任务顺序符合逻辑
4. **输出格式规范** - 符合CrewAI的输出标准
5. **错误恢复机制** - Agent失败时有重试逻辑
