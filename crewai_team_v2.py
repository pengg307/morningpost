...
def run_evening_report():
    """运行晚报系统（零 Token 模式 - CrewAI版）"""
    print("\n" + "=" * 60)
    print("📊 开始生成晚报 (CrewAI专家会诊版)...")
    print("=" * 60)
    ...
    # Step 2: 生成晚报报告（CrewAI版 - 专家团队多视角）
    from evening_report import build_evening_report_crewai, generate_comparison_summary
    crewai_report = build_evening_report_crewai(data)
    
    total_elapsed = time.time() - start_time
    
    # Step 3: 保存报告
    report_path = r"C:\Users\Pactera\projects\evening_report_crewai.txt"
    with open(report_path, "w", encoding="utf-8") as f:
